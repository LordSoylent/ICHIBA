...
#ifndef IchibaCoin_ZICHWALLET_H
#define IchibaCoin_ZICHWALLET_H
...
public:
    CzICHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZICH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZICH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //IchibaCoin_ZICHWALLET_H
