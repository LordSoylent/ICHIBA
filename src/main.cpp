...
#if defined(NDEBUG)
#error "IchibaCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uich) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 6000 && nHeight > Params().LAST_POW_BLOCK()) {
		nSubsidy = 0.1 * COIN;
	}
	else if (nHeight <= 10000 && nHeight > 6000) {
		nSubsidy = 2.5 * COIN;
	}
	...
	else if (nHeight <= 420000 && nHeight > 410000) {
		nSubsidy = 19.25 * COIN;
	}
	else if (nHeight <= 430000 && nHeight > 420000) {
		nSubsidy = 19 * COIN;
	}
	else if (nHeight <= 440000 && nHeight > 430000) {
		nSubsidy = 18.75 * COIN;
	}
	else if (nHeight <= 450000 && nHeight > 440000) {
		nSubsidy = 18.5 * COIN;
	}
	else if (nHeight <= 460000 && nHeight > 450000) {
		nSubsidy = 18.25 * COIN;
	}
	else if (nHeight <= 470000 && nHeight > 460000) {
		nSubsidy = 18 * COIN;
	}
	else if (nHeight <= 480000 && nHeight > 470000) {
		nSubsidy = 17.75 * COIN;
	}
	else if (nHeight <= 490000 && nHeight > 480000) {
		nSubsidy = 17.5 * COIN;
	}
	else if (nHeight <= 500000 && nHeight > 490000) {
		nSubsidy = 17.25 * COIN;
	}
	else if (nHeight <= 510000 && nHeight > 500000) {
		nSubsidy = 17 * COIN;
	}
	else if (nHeight <= 520000 && nHeight > 510000) {
		nSubsidy = 16.75 * COIN;
	}
	else if (nHeight <= 530000 && nHeight > 520000) {
		nSubsidy = 16.5 * COIN;
	}
	else if (nHeight <= 540000 && nHeight > 530000) {
		nSubsidy = 16.25 * COIN;
	}
	else if (nHeight <= 550000 && nHeight > 540000) {
		nSubsidy = 16 * COIN;
	}
	else if (nHeight <= 560000 && nHeight > 550000) {
		nSubsidy = 15.75 * COIN;
	}
	else if (nHeight <= 570000 && nHeight > 560000) {
		nSubsidy = 15.5 * COIN;
	}
	else if (nHeight <= 580000 && nHeight > 570000) {
		nSubsidy = 15.25 * COIN;
	}
	else if (nHeight <= 590000 && nHeight > 580000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight <= 600000 && nHeight > 590000) {
		nSubsidy = 14.75 * COIN;
	}
	else if (nHeight <= 610000 && nHeight > 600000) {
		nSubsidy = 14.5 * COIN;
	}
	else if (nHeight <= 620000 && nHeight > 610000) {
		nSubsidy = 14.25 * COIN;
	}
	else if (nHeight <= 630000 && nHeight > 620000) {
		nSubsidy = 14 * COIN;
	}
	else if (nHeight <= 640000 && nHeight > 630000) {
		nSubsidy = 13.75 * COIN;
	}
	else {
		nSubsidy = 12.5 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from IchibaCoin
...
{
	RenameThread("ichibacoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// IchibaCoin: We use certain sporks during IBD, so check to see if they are
