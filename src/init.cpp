...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ICH and zICH money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ICH/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ICH/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ICH/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-ichstake=<n>", strprintf(_("Enable or disable staking functionality for ICH inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zichstake=<n>", strprintf(_("Enable or disable staking functionality for zICH inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzich=<n>", strprintf(_("Enable automatic wallet backups triggered after each zICH minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zichbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zICH backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeichibacoinamount=<n>     " + strprintf(_("Keep N ICH anonymized (default: %u)"), 0) + "\n";
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzICHWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zICHWallet
        uiInterface.InitMessage(_("Syncing zICH wallet..."));

        bool fEnableZIchBackups = GetBoolArg("-backupzich", true);
        pwalletMain->setZIchAutoBackups(fEnableZIchBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zichTracker->Init();
...
//    if (nAnonymizeIchibaCoinAmount < 2) nAnonymizeIchibaCoinAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1ICH+1000 == (.1ICH+100)*10
       10ICH+10000 == (1ICH+1000)*10
    */
