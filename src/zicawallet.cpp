...
CzICAWallet::CzICAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zica seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZICASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZICASeed_deprecated()) {
                    LogPrintf("%s: Updated zICA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zica seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zica wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzICAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZICACount(nCountLastUsed);
    else if (!walletdb.ReadZICACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzICAWallet::Lock()
{
    seedMaster = 0;
}

void CzICAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzICAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZICA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzICAWallet::LoadMintPoolFromDB()
{
...
void CzICAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzICAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzICAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zicaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzICAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZICA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zicaTracker which also adds to database
    pwalletMain->zicaTracker->Add(dMint, true);
...
void CzICAWallet::SeedToZICA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzICAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzICAWallet::GenerateDeterministicZICA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzICAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzICAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
