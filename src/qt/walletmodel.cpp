...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ICH.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzICHReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzICHReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzICHReset.connect(boost::bind(NotifyzICHReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzICHReset.disconnect(boost::bind(NotifyzICHReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zichTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
