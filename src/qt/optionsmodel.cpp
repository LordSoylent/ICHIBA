...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ICH);
    ...
    if (!settings.contains("nAnonymizeIchibaCoinAmount"))
        settings.setValue("nAnonymizeIchibaCoinAmount", 1000);

    nAnonymizeIchibaCoinAmount = settings.value("nAnonymizeIchibaCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIchibaCoinAmount"))
        SoftSetArg("-anonymizeichibacoinamount", settings.value("nAnonymizeIchibaCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ichibacoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIchibaCoinAmount:
            return QVariant(nAnonymizeIchibaCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIchibaCoinAmount:
            nAnonymizeIchibaCoinAmount = value.toInt();
            settings.setValue("nAnonymizeIchibaCoinAmount", nAnonymizeIchibaCoinAmount);
            emit anonymizeIchibaCoinAmountChanged(nAnonymizeIchibaCoinAmount);
            break;
        case CoinControlFeatures:
