...
bool CIchStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CIchStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CIchStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CIchStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CIchStake::GetModifier(uint64_t& nStakeModifier)
{
...
CBlockIndex* CIchStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
