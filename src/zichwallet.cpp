...
CzICHWallet::CzICHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zich seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZICHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZICHSeed_deprecated()) {
                    LogPrintf("%s: Updated zICH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zich seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zich wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzICHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZICHCount(nCountLastUsed);
    else if (!walletdb.ReadZICHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzICHWallet::Lock()
{
    seedMaster = 0;
}

void CzICHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzICHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZICH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzICHWallet::LoadMintPoolFromDB()
{
...
void CzICHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzICHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzICHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zichTracker->HasPubcoinHash(pMint.first)) {
...
bool CzICHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZICH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zichTracker which also adds to database
    pwalletMain->zichTracker->Add(dMint, true);
...
void CzICHWallet::SeedToZICH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzICHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzICHWallet::GenerateDeterministicZICH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzICHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzICHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
