...
#ifndef IchibaCoin_ZICAWALLET_H
#define IchibaCoin_ZICAWALLET_H
...
public:
    CzICAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZICA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZICA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //IchibaCoin_ZICAWALLET_H
